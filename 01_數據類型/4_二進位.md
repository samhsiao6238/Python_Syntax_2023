...
2. bin() 是一個內建函數，用於將一個整數轉換為二進制字符串，返回的字符串以 `0b` 開頭表示它是二進制的。
3. 二進制數字以 `0b` 或 `0B` 開頭。
4. `bytes` 不可變、`bytearray` 可變。
5. `memoryview` 可訪問二進位數據的內存區域。

<br>

## 嚴謹定義

_二進制數據、二進制字符串 和 二進制數值是不同的概念_
...
_以下分別說明 `bytes`、`bytearray` 和 `memoryvie` 並比較其與一般列表的差異_
...
3. bytes 物件可以用 decode() 方法解碼
   ...
   _字節串 byte string_
    ...
    ```python
    # b 前綴表示該字串是一個字節串，而不是一個普通的 Unicode 字串
    b2 = b'hello'
